...
public:
  ...
  void get_origin(float &x, float &y) { x = xorig; y = yorig; }
  ...
  float get_length() { return (length1+length2); }
  ...
  Streamline* copy();
  float& xs(int index) { return pts[index].x; }
  float& ys(int index) { return pts[index].y; }
...
public:
  ...
  Bundle* copy();
  Streamline *get_line(int index) { return (lines[index]); }
